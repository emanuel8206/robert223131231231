...
function renderNametags()
    local identificators = exports.identificador:getIdentificators() or {}
	if (show) then
						...
						if not getElementData(player, "reconx") and not getElementData(player, "freecam:state") and not (getElementAlpha(player) < 255) then
							...
							if not (collision) or aimsSniper() or (reconx) then
								...
								if (sx) and (sy) then
									...
									if poli == true then
										dxDrawImage(sx-offset+xpos,oldsy,picxsize,picysize,"images/new/badge5.png")
										xpos = xpos + picxsize
									end
									local medico = getElementData(player, "dutymd")
									if medico == true then
										dxDrawImage(sx-offset+xpos,oldsy,picxsize,picysize,"images/new/badge3.png")
										xpos = xpos + picxsize
									end
									local meca = getElementData(player, "dutymeca")
									if meca == true then
										dxDrawImage(sx-offset+xpos,oldsy,picxsize,picysize,"images/new/badge4.png")
										xpos = xpos + picxsize
									end
									local gob = getElementData(player, "dutygob")
									if gob == true then
										dxDrawImage(sx-offset+xpos,oldsy,picxsize,picysize,"images/new/badge7.png")
										xpos = xpos + picxsize
									end
									local jus = getElementData(player, "dutyjus")
									if jus == true then
										dxDrawImage(sx-offset+xpos,oldsy,picxsize,picysize,"images/new/badge6.png")
										xpos = xpos + picxsize
									end
									...
									local playerAccount = getElementData( player, "player.accountName" )
									if not identificators[playerAccount] then
                                        masked = true  									
									end
									...
									if not tinted then
										...
										if identificators[playerAccount] then
										    name = identificators[playerAccount]
										end
										local vehicle = getPedOccupiedVehicle( player )
								...
								if (sx) and (sy) then
									...
									if (sx) and (sy) and oldsy then
										...
										local rectX = sx - offset
										local rectY = sy
										local rectWidth = textWidth + 10
										local rectHeight = 20 / distance
